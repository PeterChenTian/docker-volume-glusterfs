...
import (
	...
	"github.com/PeterChenTian/go-plugins-helpers/volume"
)
...
func (d glusterfsDriver) Mount(r volume.MountRequest) volume.Response {
	...
	if ok && s.connections > 0 {
		s.connections++
		log.Printf("Mounting volume %s exist, and count = %d\n", r.Name, s.connections)
...
func (d *glusterfsDriver) mountVolume(name, destination string) error {
	...
	cmd := fmt.Sprintf("mount -t glusterfs gfshost1:%s %s", name, destination)
	log.Printf("cmd %s\n", cmd)
